...
from ..utils import determine_ext, int_or_none, parse_iso8601, url_or_none
...
class ManyVidsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def convert_string_suffix_num(num_str):
            multipliers = {'k': 1000, 'm': 1000000}
            for k, v in multipliers.items():
                if num_str[-1] == k:
                    return int(num_str[:-1]) * v
            return int(num_str)

        title = info.get('title')
        uploader = traverse_obj(info, ('model', 'displayName'))
        description = info.get('description')
        likes = convert_string_suffix_num(info.get('likes'))
        views = convert_string_suffix_num(info.get('views'))
        thumbnail = url_or_none(info.get('thumbnail')) or url_or_none(info.get('screenshot'))
